...
- Permissive: `mit`, `apache-2.0`, `bsd-2-clause`, `bsd-3-clause`, `isc`, `bsl-1.0`
...
For anything else, pass named updates to `stamp_update()`. Each value is a new
value or a function of the field's current value:
...
Bundle a set of edits with `stamp_edits()` to reuse them, and point any update
at a directory to apply it to every stamped file at once:

```{r}
#| label: existing-header-batch
#| eval: false
edits <- stamp_edits(
